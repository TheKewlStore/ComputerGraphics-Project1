...
char* type_names[] = {"invalid", "char", "short", "int", "uchar", "ushort", "uint", "float", "double",};
...
void binary_get_element(PlyFile* plyfile, char* elem_ptr) {
    ...
    for (j = 0; j < elem->nprops; j++) {
        ...
        if (prop->is_list) {       /* a list */
            ...
            /* get and store the number of items in the list */
            get_binary_item(fp, prop->count_external, &int_val, &uint_val, &double_val, plyfile->file_type);
            if (store_it) {
                item = elem_data + prop->count_offset;
                store_item(item, prop->count_internal, int_val, uint_val, double_val);
            }
            ...
            if (list_count == 0) {
                ...
                /* read items and store them into the array */
                for (k = 0; k < list_count; k++) {
                    get_binary_item(fp, prop->external_type, &int_val, &uint_val, &double_val, plyfile->file_type);
                    if (store_it) {
                        store_item(item, prop->internal_type, int_val, uint_val, double_val);
                        item += item_size;
                    }
...
void get_binary_item(FILE* fp, int type, int* int_val, unsigned int* uint_val, double* double_val, int file_type) {
    char c[8];
    char tmp;
    void* ptr;
